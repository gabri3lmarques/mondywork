...
try {
    ...
    $where = "WHERE v.status = 'ativa' AND v.is_nao_listada = 0 AND v.origem = 'exterior'";
    $params = [];
    if ($filterModelo) {
    ...
    $campos = "v.id, v.vaga_id_externo, v.titulo, v.empresa, v.localizacao, v.modelo_trabalho, v.url_vaga, v.resumo, DATE_FORMAT(v.publicado_em, '%d/%m/%Y') as publicado_em";
  ...
  <div class="nav-inner">
    ...
    <div class="nav-auth-section">
      ...
      <div id="nav-user-dropdown" class="nav-user-dropdown-container" <?= !$currentUser ? 'style="display:none;"' : '' ?>>
        <button type="button" class="nav-user-trigger" id="nav-user-trigger" aria-label="User menu">
...
<?php endif; ?>
          ...
          <div class="job-card-interactions" data-vaga-id="<?= (int)$v['id'] ?>">
            <div class="job-card-reactions">
              <button type="button" class="card-reaction-btn" data-tipo="like" data-vaga-id="<?= (int)$v['id'] ?>" title="Like"><span class="card-reaction-emoji">👍</span><span class="card-reaction-count"></span></button>
              <button type="button" class="card-reaction-btn" data-tipo="dislike" data-vaga-id="<?= (int)$v['id'] ?>" title="Dislike"><span class="card-reaction-emoji">👎</span><span class="card-reaction-count"></span></button>
              <button type="button" class="card-reaction-btn" data-tipo="love" data-vaga-id="<?= (int)$v['id'] ?>" title="Loved it"><span class="card-reaction-emoji">❤️</span><span class="card-reaction-count"></span></button>
              <button type="button" class="card-reaction-btn" data-tipo="angry" data-vaga-id="<?= (int)$v['id'] ?>" title="Angry"><span class="card-reaction-emoji">😡</span><span class="card-reaction-count"></span></button>
            </div>
          ...
          <div class="job-card-comments-accordion" id="card-comments-<?= (int)$v['id'] ?>" style="display:none;" data-vaga-id="<?= (int)$v['id'] ?>">
            ...
            <div class="card-comments-list" id="card-comments-list-<?= (int)$v['id'] ?>">
              <div class="card-comments-empty-notice">Loading comments...</div>
            </div>
          </div>
        </article>
<?php endforeach; ?>
...
<script src="/js/app-exterior.js?v=2.3.0"></script>
...
<div class="auth-modal-overlay" id="auth-modal">
  <div class="auth-modal-card">
    ...
    <div class="auth-modal-header">
      ...
      <p class="auth-modal-subtitle">Access your account to react and comment on jobs</p>
    </div>

    <div class="auth-tabs">
      <button type="button" class="auth-tab-btn active" data-tab="login" id="tab-login-btn">Sign In</button>
      <button type="button" class="auth-tab-btn" data-tab="register" id="tab-register-btn">Create Account</button>
    </div>
    ...
    <form id="auth-form-login">
      <div class="auth-form-group">
        ...
        <input type="email" id="login-email" class="auth-form-input" placeholder="your.email@example.com" required autocomplete="email">
      </div>
      <div class="auth-form-group">
        <label class="auth-form-label" for="login-senha">Password</label>
    ...
    <form id="auth-form-register" style="display: none;" enctype="multipart/form-data">
      <div class="auth-form-group">
        <label class="auth-form-label" for="reg-nome">Full Name *</label>
        <input type="text" id="reg-nome" class="auth-form-input" placeholder="e.g. John Doe" required autocomplete="name">
      </div>
      <div class="auth-form-group">
        <label class="auth-form-label" for="reg-email">Email *</label>
        <input type="email" id="reg-email" class="auth-form-input" placeholder="your.email@example.com" required autocomplete="email">
      </div>
      <div class="auth-form-group">
        <label class="auth-form-label" for="reg-senha">Password * (min. 6 characters)</label>
      ...
      <div class="auth-avatar-section">
        ...
        <div class="auth-avatar-info">
          <div class="auth-avatar-title">Profile Photo <span style="font-weight:400;color:#64748b;">(optional)</span></div>
          <div class="auth-avatar-hint">Square crop & auto compression (max. 128KB). Fallback uses name initial.</div>
          <label for="reg-avatar-input" class="auth-avatar-upload-btn">Choose photo...</label>
