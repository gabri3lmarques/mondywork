...
if ($isLoggedIn && isset($_GET['action']) && $_GET['action'] === 'stream_sync') {
    ...
    $syncScript = __DIR__ . '/sync.php';
    if (!file_exists($syncScript)) {
        echo "data: " . json_encode(['error' => 'Arquivo sync.php não encontrado.']) . "\n\n";
        echo "data: " . json_encode(['done' => true]) . "\n\n";
        flush();
        exit;
    }

    $provider = strtolower(trim($_GET['provider'] ?? 'all'));
    $origem = strtolower(trim($_GET['origem'] ?? 'all'));
    $startLote = max(1, (int)($_GET['start_lote'] ?? 1));
    $empresa = trim($_GET['empresa'] ?? '');

    $args = [];
    if ($provider !== 'all' && in_array($provider, ['inhire', 'ashby', 'greenhouse', 'senior'])) {
        $args[] = '--provider=' . escapeshellarg($provider);
    }
    if ($origem !== 'all' && in_array($origem, ['nacional', 'exterior'])) {
        $args[] = '--origem=' . escapeshellarg($origem);
    }
    if ($startLote > 1) {
        $args[] = '--start-lote=' . $startLote;
    }
    if ($empresa !== '') {
        $args[] = '--empresa=' . escapeshellarg($empresa);
    }

    $argString = !empty($args) ? ' ' . implode(' ', $args) : '';
    $phpBin = 'php';
    if (defined('PHP_BINARY') && PHP_BINARY && strpos(PHP_BINARY, 'fpm') === false && is_executable(PHP_BINARY)) {
        $phpBin = PHP_BINARY;
    } else {
        foreach (['/usr/local/bin/php', '/usr/bin/php', '/bin/php'] as $candidate) {
    ...
    $cmd = escapeshellcmd($phpBin) . ' ' . escapeshellarg($syncScript) . $argString . ' 2>&1';
    ...
    if (is_resource($process)) {
        ...
        echo "data: " . json_encode(['error' => 'Não foi possível iniciar o processo de sincronização.']) . "\n\n";
        echo "data: " . json_encode(['done' => true]) . "\n\n";
        flush();
    }
    exit;
...
.sync-modal-window {
  ...
  overflow: hidden;
  animation: syncModalFadeIn 0.2s ease-out;
}
...
.sync-badge.idle { background: #334155; color: #cbd5e1; }
.sync-badge.running { background: #065f46; color: #34d399; animation: pulseGlow 1.5s infinite; }
.sync-badge.done { background: #1e3a8a; color: #93c5fd; }
.sync-badge.error { background: #881337; color: #fda4af; }
@keyframes pulseGlow {
  0%, 100% { opacity: 1; }
  50% { opacity: 0.6; }
}
...
/* Log Colors */
.log-nova { color: #4ade80; font-weight: 600; }
.log-ign { color: #64748b; }
...
.sync-footer-left, .sync-footer-right {
  display: flex;
  align-items: center;
  gap: 10px;
}
...
.btn-sync-action.start {
  background: #10b981;
  color: #fff;
}
.btn-sync-action.start:hover {
  background: #059669;
}
.btn-sync-action.stop {
  background: #ef4444;
  color: #fff;
}
.btn-sync-action.stop:hover {
  background: #dc2626;
}
.btn-sync-action.clear {
  background: #334155;
  color: #cbd5e1;
  border-color: #475569;
}
.btn-sync-action.clear:hover {
  background: #475569;
  color: #fff;
}
.btn-sync-action.close {
  background: #334155;
  color: #cbd5e1;
  border-color: #475569;
}
.btn-sync-action.close:hover {
  background: #475569;
  color: #fff;
}
.sync-autoscroll-toggle {
  display: inline-flex;
  align-items: center;
  gap: 6px;
  font-size: 12px;
...
.sync-select:focus, .sync-input:focus {
  border-color: #6366f1;
}
.sync-select option {
  background: #0f172a;
...
<nav class="nav admin-nav">
  <div class="nav-inner">
    ...
    <div class="nav-links">
      <?php if ($isLoggedIn): ?>
        <button type="button" class="btn-nav-sync" id="btn-open-sync-modal" title="Executar worker de sincronização">
          ⚡ Rodar Sync
        </button>
      <?php endif; ?>
...
<main class="admin-main">
  <div class="admin-nav-bar">
    <div class="admin-tabs">
      ...
      <button type="button" class="admin-tab admin-tab-btn-sync" id="tab-btn-open-sync" title="Abrir terminal de sincronização">
        ⚡ Rodar Sync
      </button>
    </div>
  </div>
...
<!-- Modal Terminal de Sincronização em Tempo Real -->
<div id="sync-modal-overlay" class="sync-modal-overlay" style="display:none;">
  <div class="sync-modal-window">
    <div class="sync-modal-header">
      <div class="sync-modal-title-group">
        <span class="sync-terminal-icon">⚡</span>
        <div>
          <h2 class="sync-modal-title">Sincronização de Vagas (sync.php)</h2>
          <div class="sync-modal-subtitle">Execução em tempo real com controle de lote e provedor</div>
        </div>
      </div>
      <div class="sync-modal-actions">
        <span id="sync-status-badge" class="sync-badge idle">⚪ Parado</span>
        <span id="sync-timer" class="sync-timer" style="display:none;">00:00</span>
        <button type="button" class="sync-btn-close" id="btn-close-sync-modal" aria-label="Fechar">&times;</button>
      </div>
    </div>

    <!-- Barra de Opções e Retomada de Lote -->
    <div class="sync-options-bar">
      <div class="sync-option-item">
        <label for="sync-opt-provider">Fonte / ATS</label>
        <select id="sync-opt-provider" class="sync-select">
      ...
      <div class="sync-option-item">
        <label for="sync-opt-origem">Origem</label>
        <select id="sync-opt-origem" class="sync-select">
          <option value="all">Nacional + Exterior</option>
          <option value="nacional">Apenas Nacional</option>
          <option value="exterior">Apenas Exterior</option>
        </select>
      </div>
      <div class="sync-option-item" style="width: 130px;">
        <label for="sync-opt-lote">Iniciar no Lote</label>
        <input type="number" id="sync-opt-lote" class="sync-input" value="1" min="1" placeholder="Ex: 18">
      </div>
      <div class="sync-option-item" style="flex: 1; min-width: 160px;">
        <label for="sync-opt-empresa">Filtrar Empresa (opcional)</label>
...
Configure os filtros acima se desejar (ou deixe padrão) e clique em "Iniciar Sincronização".</pre>
    </div>

    <div class="sync-modal-footer">
      <div class="sync-footer-left">
        <button type="button" class="btn-sync-action start" id="btn-start-sync">
      ...
      <div class="sync-footer-right">
        <label class="sync-autoscroll-toggle">
          <input type="checkbox" id="sync-autoscroll-check" checked>
          <span>Auto-scroll</span>
        </label>
        <button type="button" class="btn-sync-action close" id="btn-dismiss-sync-modal">Fechar</button>
      </div>
    </div>
  </div>
</div>
...
(function() {
  ...
  var btnOpenTop = document.getElementById('btn-open-sync-modal');
  var btnOpenTab = document.getElementById('tab-btn-open-sync');
  var btnClose = document.getElementById('btn-close-sync-modal');
  var btnDismiss = document.getElementById('btn-dismiss-sync-modal');
  var btnStart = document.getElementById('btn-start-sync');
  var btnStop = document.getElementById('btn-stop-sync');
  var btnClear = document.getElementById('btn-clear-sync-logs');
  var terminalContainer = document.getElementById('sync-terminal-container');
  var terminalOutput = document.getElementById('sync-terminal-output');
  var badge = document.getElementById('sync-status-badge');
  var timerEl = document.getElementById('sync-timer');
  var autoscrollCheck = document.getElementById('sync-autoscroll-check');

  var optProvider = document.getElementById('sync-opt-provider');
  var optOrigem = document.getElementById('sync-opt-origem');
  ...
  function closeModal() {
    if (!overlay) return;
    if (isRunning) {
      if (!confirm('A sincronização está em andamento. Deseja fechar a visualização? (A execução no servidor continuará até finalizar)')) {
        return;
      }
      stopListening();
    }
  ...
  function clearLogs(resetPlaceholder) {
    if (!terminalOutput) return;
    if (resetPlaceholder) {
      terminalOutput.innerHTML = 'Aguardando início da sincronização...\nConfigure os filtros acima se desejar (ou deixe padrão) e clique em "Iniciar Sincronização".';
      if (badge) {
  ...
  function formatLine(line) {
    ...
    if (escaped.includes('[NOVA]')) {
      return '<span class="log-nova">' + escaped + '</span>';
    } else if (escaped.includes('[IGNORADA]') || escaped.includes('[MANTIDA]')) {
      return '<span class="log-ign">' + escaped + '</span>';
    } else if (escaped.includes('[ANTIGAS]') || escaped.includes('[ORFAOS]') || escaped.includes('[MODO PARCIAL]')) {
      return '<span class="log-warn">' + escaped + '</span>';
    } else if (escaped.includes('[ERRO]') || escaped.includes('[ERRO FATAL]')) {
      return '<span class="log-err">' + escaped + '</span>';
    } else if (escaped.includes('Iniciando sincronizacao') || escaped.includes('Sincronizacao finalizada') || escaped.includes('[CONFIG]')) {
      return '<span class="log-info">' + escaped + '</span>';
    } else if (escaped.includes('--- InHire') || escaped.includes('--- Ashby') || escaped.includes('--- Greenhouse') || escaped.includes('--- Senior')) {
      return '<span class="log-section">' + escaped + '</span>';
    }
    return escaped;
  }

  function appendLog(rawText) {
    if (!rawText || !terminalOutput) return;
    var lines = rawText.split('\n');
    for (var i = 0; i < lines.length; i++) {
      if (i === lines.length - 1 && lines[i] === '') continue;
      var formatted = formatLine(lines[i]);
  ...
  function startSync() {
    ...
    var provider = optProvider ? optProvider.value : 'all';
    var origem = optOrigem ? optOrigem.value : 'all';
    var lote = optLote ? Math.max(1, parseInt(optLote.value, 10) || 1) : 1;
    var empresa = optEmpresa ? optEmpresa.value.trim() : '';
    ...
    evtSource = new EventSource('admin.php?' + params.toString());
      ...
      try {
        ...
        if (data.done) {
          ...
          if (badge && badge.textContent !== '🔴 Erro') {
            badge.className = 'sync-badge done';
    ...
    evtSource.onerror = function() {
      ...
      if (badge && badge.textContent !== '🔴 Erro') {
        badge.className = 'sync-badge done';
        badge.textContent = 'Conexão finalizada';
      }
    };
  }
